  ...
  return (
    <main className="dashboard-page">
      ...
      <section className="shell stat-grid">
        ...
        <article><Heart /><span>お気に入り</span><strong>{favorites.length}<small> 本</small></strong><p>更新通知 ON</p></article>
        <article><Clock3 /><span>届けた応援</span><strong>{totalSupport}<small> pt</small></strong><p>{feedback.length || 2}件のフィードバック</p></article>
      </section>
      <section className="shell dashboard-columns">
        ...
        <div className="dashboard-panel"><div className="panel-heading"><div><span className="kicker">LATEST UPDATES</span><h2>お気に入りの更新</h2></div></div><div className="mini-updates">{updates.slice(0, 3).map((item) => <article key={item.game}><i /><div><b>{item.game} <span>{item.version}</span></b><p>{item.text}</p><time>{item.date}</time></div></article>)}</div></div>
      </section>
