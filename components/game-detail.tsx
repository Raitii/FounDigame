...
import { ArrowLeft, Check, ChevronRight, Gamepad2, Heart, MessageSquareText, Share2, Sparkles, Users } from "lucide-react";
...
export function GameDetail({ game }: { game: Game }) {
  const { favorites, toggleFavorite, points, allocations, support } = useDemo();
    ...
    <main className="detail-page" style={{ "--accent": game.accent } as React.CSSProperties}>
      ...
      <section className="shell detail-layout">
        <div className="detail-content">
          ...
          <article className="content-block wanted"><MessageSquareText /><div><span className="kicker">CREATOR WANTS TO KNOW</span><h2>こんな声を待っています</h2><ul><li>最初の5分で、次に何をすればいいか迷わなかった？</li><li>集める・作るの繰り返しは気持ちよかった？</li><li>もっと会ってみたい住人はどんなキャラクター？</li></ul></div></article>
        </div>
        <aside className="support-panel">
          <div className="support-icon"><Sparkles /></div><span className="kicker">SUPPORT THIS GAME</span><h2>続きを、応援する。</h2><p>応援ポイントは、開発者への期待を伝えるサインです。</p><div className="wallet-row"><span>使えるポイント</span><b>{points} pt</b></div><div className="amounts">{[10, 50, 100].map((n) => <button className={amount === n ? "active" : ""} onClick={() => setAmount(n)} key={n}>{n}</button>)}</div><button className="button support-button" onClick={handleSupport}><Sparkles size={17} /> {amount}pt 応援する</button><small>この作品に応援済み：{allocations[game.slug] ?? 0} pt</small>{notice && <div className="toast"><Check size={16} /> {notice}</div>}</aside>
      </section>
